    ...
    suspend fun signup(email: String, password: String): Result<FirebaseUser> {
        return try {
            ...
            try {
                ...
                println(" Failed to send verification email: Email service temporarily unavailable")
    ...
    suspend fun resendVerificationEmail(): Result<Unit> {
        return try {
            ...
            if (user != null && !user.isEmailVerified) {
                user.sendEmailVerification().await()
                println(" Verification email resent to: ${user.email}")
                Result.success(Unit)
            } else if (user == null) {
                println(" No user logged in")
                Result.failure(Exception("No user is currently logged in."))
            } else {
                println("⚠ User email already verified")
                Result.failure(Exception("User email is already verified."))
            }
        } catch (e: Exception) {
            println(" Failed to resend verification email: ${e.message}")
            Result.failure(Exception("Failed to resend email: ${e.message}"))
        }
    }
