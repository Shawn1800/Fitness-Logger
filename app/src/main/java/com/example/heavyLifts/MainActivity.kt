...
fun MainNavigation() {
    ...
    val db =remember { ExerciseDatabase.getInstance(context)}

    val workoutRepository = remember {  WorkoutRepository(db.workoutEntryEntityDao())}
    val exerciseRepository = remember {  ExerciseRepository(
        db.exerciseDao(),
        workoutEntryEntityDao = db.workoutEntryEntityDao()
    )}
    val oneRepMaxRepository =remember {OneRepMaxRepository(db.oneRepMaxEntityDao(), workoutEntryEntityDao = db.workoutEntryEntityDao())}
